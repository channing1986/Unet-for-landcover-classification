...
def FCN_Vgg16_32s_smallinput(input_shape=None, weight_decay=0., batch_momentum=0.9, batch_shape=None, classes=21):
    if batch_shape:
        img_input = Input(batch_shape=batch_shape)
        image_size = batch_shape[1:3]
    else:
        img_input = Input(shape=input_shape)
        image_size = input_shape[0:2]
    # Block 1//input is 32*32
    x = Conv2D(64, (3, 3), activation='relu', padding='same', name='block1_conv1', kernel_regularizer=l2(weight_decay))(img_input)
    x = Conv2D(64, (3, 3), activation='relu', padding='same', name='block1_conv2', kernel_regularizer=l2(weight_decay))(x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='block1_pool')(x)

    # Block 2//input is 16*16
    x = Conv2D(128, (3, 3), activation='relu', padding='same', name='block2_conv1', kernel_regularizer=l2(weight_decay))(x)
    x = Conv2D(128, (3, 3), activation='relu', padding='same', name='block2_conv2', kernel_regularizer=l2(weight_decay))(x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='block2_pool')(x)

    # Block 3//input is 8*8
    x = Conv2D(256, (3, 3), activation='relu', padding='same', name='block3_conv1', kernel_regularizer=l2(weight_decay))(x)
    x = Conv2D(256, (3, 3), activation='relu', padding='same', name='block3_conv2', kernel_regularizer=l2(weight_decay))(x)
    x = Conv2D(256, (3, 3), activation='relu', padding='same', name='block3_conv3', kernel_regularizer=l2(weight_decay))(x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='block3_pool')(x)# output is 4*4

    # Block 4//input is 4*4
    #x = Conv2D(512, (3, 3), activation='relu', padding='same', name='block4_conv1', kernel_regularizer=l2(weight_decay))(x)
    #x = Conv2D(512, (3, 3), activation='relu', padding='same', name='block4_conv2', kernel_regularizer=l2(weight_decay))(x)
    #x = Conv2D(512, (3, 3), activation='relu', padding='same', name='block4_conv3', kernel_regularizer=l2(weight_decay))(x)
    #x = MaxPooling2D((2, 2), strides=(2, 2), name='block4_pool')(x)# output is 2*2?
    ...
    x = BilinearUpSampling2D(size=(8, 8))(x)# the output upsaml is 8 times larger.
...
def FCN_Resnet50_edge(input_shape = None, weight_decay=0., batch_momentum=0.9, batch_shape=None, classes=2):
    if batch_shape:
        img_input = Input(batch_shape=batch_shape)
        image_size = batch_shape[1:3]
    else:
    ...
    side1=side_block(3, [64, 64, 1], stage=1, block='side')(x)
    side1= BilinearUpSampling2D(size=(2, 2))(side1)
    #side1=UpSampling2D(size=(2, 2))(side1)
    x = MaxPooling2D((3, 3), padding='same', strides=(2, 2))(x)

    x = conv_block(3, [64, 64, 256], stage=2, block='a', strides=(1, 1))(x)
    x = identity_block(3, [64, 64, 256], stage=2, block='b')(x)
    x = identity_block(3, [64, 64, 256], stage=2, block='c')(x)
    side2=side_block(3, [256, 256, 1], stage=2, block='side')(x)
    side2= BilinearUpSampling2D(size=(4, 4))(side2)

    x = conv_block(3, [128, 128, 512], stage=3, block='a')(x)
    x = identity_block(3, [128, 128, 512], stage=3, block='b')(x)
    x = identity_block(3, [128, 128, 512], stage=3, block='c')(x)
    x = identity_block(3, [128, 128, 512], stage=3, block='d')(x)
    side3=side_block(3, [512, 512, 1], stage=3, block='side')(x)
    side3= BilinearUpSampling2D(size=(8, 8))(side3)
    ...
    x = identity_block(3, [256, 256, 1024], stage=4, block='e')(x)
    x = identity_block(3, [256, 256, 1024], stage=4, block='f')(x)
    side4=side_block(3, [1024, 1024, 1], stage=4, block='side')(x)
    side4= BilinearUpSampling2D(size=(16, 16))(side4)


    x = conv_block(3, [512, 512, 2048], stage=5, block='a')(x)
    x = identity_block(3, [512, 512, 2048], stage=5, block='b')(x)
    x = identity_block(3, [512, 512, 2048], stage=5, block='c')(x)
    side5=side_block(3, [2048, 2048, 1], stage=5, block='side')(x);
    side5 = BilinearUpSampling2D(size=(32, 32))(side5)
    ...
    #x = BilinearUpSampling2D(size=(32, 32))(x)
    #x = top(x, input_shape, classes, 'sigmoid', weight_decay)

    model = Model(img_input, x)
    #weights_path = os.path.expanduser(os.path.join('~', '.keras/models/fcn_resnet50_weights_tf_dim_ordering_tf_kernels.h5'))
    model.load_weights('./Models/ResNet50/model.hdf5', by_name=True)
    return model
def FCN_Resnet50_edge_nosoft(input_shape = None, weight_decay=0., batch_momentum=0.9, batch_shape=None, classes=2):
    if batch_shape:
        img_input = Input(batch_shape=batch_shape)
        image_size = batch_shape[1:3]
    else:
...
def FCN_Vgg16_32s_smallinput(input_shape=None, weight_decay=0., batch_momentum=0.9, batch_shape=None, classes=21):
    ...
    # Convolutional layers transfered from fully-connected layers
    x = Conv2D(4096, (3, 3), activation='relu', padding='same', name='fc1', kernel_regularizer=l2(weight_decay))(x)
    x = Dropout(0.5)(x)
    x = Conv2D(4096, (1, 1), activation='relu', padding='same', name='fc2', kernel_regularizer=l2(weight_decay))(x)
    x = Dropout(0.5)(x)
    #classifying layer
...
def discriminator_regress(img_dim, model_name="Discriminator", use_mbd=True):
    """
    ...
    nb_filters = 64
    #img_dim=(80, 80, 5)##, this should be 
    nb_conv = int(np.round(np.log(img_dim[0]) / np.log(2)))
    nb_conv=8
    ...
    #x_flat = Flatten()(x)
    x = Dense(1,  name="disc_dense")(x)
    x=Flatten()(x)
    PatchGAN = Model(inputs=[x_input], outputs=[x], name=model_name)
    #print("PatchGAN summary")
    #PatchGAN.summary()
    return PatchGAN

def discriminator_class(img_dim, model_name="Discriminator", use_mbd=True):
    """
    Discriminator model of the DCGAN
