...
def ResNet(inp, layers):
    ...
    cnv1 = Conv2D(64, (3, 3), strides=(2, 2), padding='same', name=names[0]+str(9),
...
def interp_block(prev_layer, level, feature_map_shape, input_shape_3):
    input_shape=input_shape_3[0:2]
...
def build_pyramid_pooling_module(res, input_shape_3):
...
def build_pspnet(nb_classes, resnet_layers, input_shape, activation='sigmoid'):
    """Build PSPNet."""
    ...
    if len(input_shape)==3:
        ...
        channel=3
    inp = Input((input_shape[0], input_shape[1], channel))
    ...
    x = Conv2D(nb_classes, (1, 1), strides=(1, 1), name="final_conv_{}".format(nb_classes))(x)
