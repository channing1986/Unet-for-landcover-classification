...
def image_augmentation_test(currImg, labelMask):
    ...
    :return: post-augmentation image and ground truth data
    """
    #aug = Compose([RandomGamma(gamma_limit=(70, 130), p=0.6),GridDistortion(distort_limit=0.2,p=0.6)])
    aug = Compose([VerticalFlip(p=0.5),GridDistortion(distort_limit=0.2, p=0.5),             
             RandomRotate90(p=0.5),HorizontalFlip(p=0.5),Transpose(p=0.5)])
